      ...
      `;
            ...
            if (!u.is_admin) {
                ...
                for (const [key, label] of Object.entries(PRIV_LABELS)) {
                    const checked =
                        u.privileges && u.privileges[key] ? "checked" : "";
            ...
            <label class="admin-switch" style="transform:scale(0.85);"><input type="checkbox" data-priv="${key}" data-user="${esc(u.username)}" ${checked}><span class="admin-slider"></span></label>
                ...
                const maxMsg =
                    (u.privileges && u.privileges.max_messages_per_day) || 0;
                html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;">
          <div>
          ...
          <div class="priv-models-list" data-user="${esc(u.username)}">
            <span style="opacity:0.4;font-size:11px;">Loading models...</span>
                ...
                header.addEventListener("click", (e) => {
                    if (
                        e.target.closest(
                            ".admin-btn-delete, [data-adm-rename-user]",
                        )
                    )
                        return;
                    ...
                    if (
                        !_modelsLoaded &&
                        !privPanel.classList.contains("hidden")
                    ) {
                        _modelsLoaded = true;
                        _loadModelsForUser(
                            u.username,
            ...
            if (renameBtn) {
                renameBtn.addEventListener("click", async (e) => {
                    ...
                    const next = await uiModule.styledPrompt(
                        `Rename "${oldUsername}"`,
                        {
                    ...
                    try {
                        ...
                        if (!res.ok) {
                            uiModule.showError(
                                data.detail || "Failed to rename user",
                            );
                            return;
                        }
...
async function _loadModelsForUser(
    ...
    try {
        ...
        listEl.innerHTML = sortModelObjects(allModels)
            .map((m) => {
                const checked =
                    !restricted || allowedSet.has(m.mid) ? "checked" : "";
                return `<label>
...
async function loadEndpoints() {
    ...
    try {
        ...
        if (!Array.isArray(data) || data.length === 0) {
            ...
            if (listApi)
                listApi.innerHTML = '<div class="admin-empty">None</div>';
        ...
        const rowHtml = data.map((ep) => {
            ...
            const category =
                ep.category ||
                (_isLocalEndpoint(ep.base_url) ? "local" : "api");
            const kindLabel =
        ...
        data.forEach((ep, i) =>
            ((ep.category ||
                (_isLocalEndpoint(ep.base_url) ? "local" : "api")) === "local"
                ? localIdx
                : apiIdx
            ).push(i),
        );
              ...
              <span style="display:flex;gap:8px;align-items:center;">
                ...
                <span>${esc(m.display)}</span>
                        ...
                        const filterRows = (q) => {
                            ...
                            panel
                                .querySelectorAll("[data-ep-model-row]")
                                .forEach((row) => {
                                    row.style.display =
                                        !needle ||
                    ...
                    try {
                        const res = await fetch(
                            `/api/model-endpoints/${epId}/models`,
                            { credentials: "same-origin" },
                        );
...
function initEndpointForm() {
    ...
    function _renderPickerMenu() {
        ...
        pickerMenu.innerHTML = Array.from(provider.options)
            .map((o) => {
                const logo = o.dataset.logo
                    ? providerLogo(o.dataset.logo) || ""
                    : "";
    ...
    async function _startCodexOAuth() {
        ...
        try {
            const startRes = await fetch(
                "/api/model-endpoints/codex/oauth/device/start",
                { method: "POST", credentials: "same-origin" },
            );
            const start = await startRes.json().catch(() => ({}));
            if (!startRes.ok)
                throw new Error(start.detail || "Could not start login");
            ...
            if (hint)
                hint.innerHTML = `Opened ChatGPT Codex login. Enter code <code>${esc(code)}</code> at <a href="${uri}" target="_blank" rel="noopener" style="color:var(--accent,var(--red));">${uri}</a>. Waiting for authorization...`;
            const intervalMs = Math.max(
                1000,
                Number(start.interval || 5) * 1000,
            );
            const deadline =
                Date.now() +
                Math.max(60, Number(start.expires_in || 900)) * 1000;
            while (Date.now() < deadline) {
                await new Promise((r) => setTimeout(r, intervalMs));
                const pollRes = await fetch(
                    "/api/model-endpoints/codex/oauth/device/poll",
                    {
                        method: "POST",
                        credentials: "same-origin",
                        headers: { "Content-Type": "application/json" },
                ...
                if (poll.api_key) {
                    ...
                    if (hint)
                        hint.innerHTML = `Codex login connected${poll.account_id ? ` for account <code>${esc(poll.account_id)}</code>` : ""}. Click <b>Test</b> or <b>Add</b>.`;
                    return;
                }
            }
            ...
            if (hint)
                hint.innerHTML = `Codex login failed: ${esc(e && e.message ? e.message : String(e))}`;
        } finally {
            const latest = el("adm-codex-oauth-btn");
            if (latest) {
                latest.disabled = false;
                latest.textContent = "Connect with ChatGPT";
            }
        }
    }
    function _syncProviderHint() {
        ...
        if (_isCodexSubscriptionProvider(provider.value || urlInput.value)) {
            hint.style.display = "";
            hint.innerHTML =
                "ChatGPT Codex Subscription uses OpenAI Codex OAuth, not a regular OpenAI API key. " +
            ...
            hint.querySelector("#adm-codex-oauth-btn")?.addEventListener(
                "click",
                _startCodexOAuth,
            );
            if (keyInput)
                keyInput.placeholder =
                    "Codex OAuth token JSON or accessToken::account_id";
        } else {
    ...
    function _normalizeBaseUrl(raw) {
        ...
        u = u.replace(
            /\/(models|chat\/completions|completions|v1\/messages|responses)\/?$/i,
            "",
        );
    ...
    el("adm-epAddBtn").addEventListener("click", async () => {
        ...
        try {
            ...
            if (
                ...
                fd.append(
                    "name",
                    provider.selectedOptions[0].textContent.trim(),
                );
            }
            ...
            if (
                provider.value &&
                /openrouter\.ai|ollama\.com/i.test(provider.value)
            )
    ...
    if (copilotBtn) {
        ...
        copilotBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    status.textContent =
                        start.detail || "Failed to start login";
            ...
            const poll = async () => {
                ...
                try {
                    ...
                    if (d.status === "authorized") {
                        const n = ((d.endpoint && d.endpoint.models) || [])
                            .length;
                        done(
                            "admin-success",
                            "✓ Connected — " +
    ...
    if (localAddBtn) {
        localAddBtn.addEventListener("click", async () => {
            ...
            if (!raw) {
                msg.textContent =
                    "Enter a base URL (e.g. http://localhost:8002/v1)";
                msg.className = "admin-error";
                return;
            }
            const url = _normalizeBaseUrl(raw);
        ...
        discoverBtn.addEventListener("click", async () => {
            ...
            try {
                const sp =
                    ...
                    (await import("./spinner.js")).default;
                        ...
                        if (r.ok) {
                            try {
                                ...
                                if (dd && dd.existing) {
                                    ...
                                    added++;
                                    if (dd && dd.id)
                                        _recentlyAddedEpId = String(dd.id);
                                }
                            } catch (_) {
                                added++;
                            }
...
9. Sign in with Google, copy the URL from the error page, paste it back`,
    },
    {
        ...
        providerDropdown: {
            label: "Provider",
            targets: {
                MCP_EMAIL_IMAP_HOST: "imap",
                MCP_EMAIL_SMTP_HOST: "smtp",
            },
            options: [
                {
                    name: "Migadu",
                    imap: "imap.migadu.com",
                    smtp: "smtp.migadu.com",
                },
                {
                    name: "Fastmail",
...
async function loadBuiltinTools() {
    ...
    try {
        ...
        function _updateCatCounter(catEl) {
            ...
            const catEnabled = Array.from(catChecks).filter(
                (c) => c.checked,
            ).length;
            const counter = catEl.querySelector(".admin-tool-cat-count");
            if (counter)
                counter.textContent = catEnabled + "/" + catChecks.length;
            const catToggle = catEl.querySelector(
                "input[data-tool-cat-toggle]",
            );
        ...
        if (help) {
            ...
            helpLink.addEventListener("click", (e) => {
                ...
                helpBox.style.display =
                    helpBox.style.display === "none" ? "" : "none";
            });
    ...
    function _collectEnv() {
        if (_envKeys.length) {
            ...
            envFieldsWrap.querySelectorAll(".mcp-env-input").forEach((inp) => {
                if (inp.value.trim())
                    obj[inp.dataset.envKey] = inp.value.trim();
            });
...
async function loadRag() {
    try {
        ...
        if (files.length === 0) {
            fileList.innerHTML =
                '<div class="admin-empty">No files indexed</div>';
        } else {
...
async function loadWebhooks() {
    ...
    try {
        ...
        list.innerHTML =
            '<div class="admin-error">Failed to load webhooks</div>';
    }
}
...
function initWebhookForm() {
    el("adm-whAddBtn").addEventListener("click", async () => {
        ...
        const events = Array.from(
            modalEl.querySelectorAll(".adm-wh-event:checked"),
        )
            .map((e) => e.value)
            .join(",");
        if (!name) {
    ...
    try {
      ...
      <div class="admin-toggle-row" style="padding:0.4rem 0;border-bottom:1px solid var(--border);">
        ...
        <label class="admin-switch"><input type="checkbox" data-adm-feature="${key}" ${features[key] ? "checked" : ""}><span class="admin-slider"></span></label>
            ...
            .join("");
        container
            .querySelectorAll("input[data-adm-feature]")
            .forEach((toggle) => {
                toggle.addEventListener("change", async () => {
                    const body = {};
    ...
    fileInput.addEventListener("change", async () => {
        ...
        try {
            ...
            if (res.ok && result.ok) {
                ...
                msg.textContent =
                    result.message || result.detail || "Import failed";
                msg.className = "admin-error";
            }
        } catch (e) {
